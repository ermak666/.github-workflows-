...
export default function LessonScreen() {
  ...
  const hasLessonVoiceover = lessonAudioSource !== undefined;
  const { activeLessonId, playbackState, position, duration, playbackRate, audioError, playLesson, pause, resume, stop, setPlaybackRate, seekBy } = useLessonAudio();
  ...
  const startVoiceover = () => {
    if (!hasLessonVoiceover || !lesson) return;
    ...
    <ScreenContainer className="px-5">
      <ScrollView contentContainerStyle={{ paddingBottom: 42 }} showsVerticalScrollIndicator={false}>
        ...
        <View className="overflow-hidden rounded-[30px] border border-[#354062] bg-[#151A36] p-5 shadow-sm">
          ...
          <View className="mt-5 flex-row flex-wrap gap-2">{hasLessonVoiceover && !isCurrentVoiceover ? <Pressable accessibilityRole="button" onPress={startVoiceover} style={({ pressed }) => [{ borderRadius: 999, backgroundColor: "#E7E0FF", paddingHorizontal: 16, paddingVertical: 12 }, { opacity: pressed ? 0.8 : 1 }]}><Text className="font-bold text-primary">▶ Слушать Algieba</Text></Pressable> : null}{hasLessonVoiceover && isCurrentVoiceover && playbackState === "playing" ? <Pressable accessibilityRole="button" accessibilityLabel="Поставить озвучивание на паузу" onPress={pause} style={({ pressed }) => [{ borderRadius: 999, backgroundColor: "#E7E0FF", paddingHorizontal: 16, paddingVertical: 12 }, { opacity: pressed ? 0.8 : 1 }]}><Text className="font-bold text-primary">Ⅱ Пауза</Text></Pressable> : null}{hasLessonVoiceover && isCurrentVoiceover && playbackState === "paused" ? <Pressable accessibilityRole="button" accessibilityLabel="Продолжить озвучивание" onPress={resume} style={({ pressed }) => [{ borderRadius: 999, backgroundColor: "#E7E0FF", paddingHorizontal: 16, paddingVertical: 12 }, { opacity: pressed ? 0.8 : 1 }]}><Text className="font-bold text-primary">▶ Продолжить</Text></Pressable> : null}{hasLessonVoiceover && isCurrentVoiceover ? <Pressable accessibilityRole="button" accessibilityLabel="Полностью остановить озвучивание" onPress={stop} style={({ pressed }) => [{ borderRadius: 999, borderWidth: 1, borderColor: "#FFB9C5", backgroundColor: "#3B1724", paddingHorizontal: 16, paddingVertical: 12 }, { opacity: pressed ? 0.8 : 1 }]}><Text className="font-bold text-[#FFE2E8]">■ Стоп</Text></Pressable> : null}<Pressable accessibilityRole="button" onPress={() => setShowBookmarks((value) => !value)} style={({ pressed }) => [{ borderRadius: 999, borderWidth: 1, borderColor: "#6872AA", backgroundColor: "#242B4D", paddingHorizontal: 16, paddingVertical: 12 }, { opacity: pressed ? 0.8 : 1 }]}><Text className="font-bold text-white">В закладки</Text></Pressable></View>
          {hasLessonVoiceover ? <Text className="mt-3 text-xs leading-4 text-[#D8DDEA]">Algieba — один общий плеер: запуск нового урока остановит предыдущую запись.</Text> : null}
          {audioError ? <Text className="mt-3 text-xs leading-4 text-[#FFD7DE]">{audioError}</Text> : null}
          {hasLessonVoiceover && isCurrentVoiceover ? <View className="mt-4 rounded-2xl bg-[#242B4D] p-4"><View className="flex-row items-center justify-between"><Text className="text-xs font-bold text-[#C9C6FF]">{formatAudioTime(position)} / {formatAudioTime(duration)}</Text><Text className="text-xs font-bold text-[#C9C6FF]">{playbackRate.toFixed(1)}×</Text></View><View className="mt-2 h-2 overflow-hidden rounded-full bg-[#111426]"><View style={{ width: `${audioPercent}%` }} className="h-full rounded-full bg-[#A993FF]" /></View><View className="mt-3 flex-row flex-wrap gap-2"><Pressable accessibilityRole="button" onPress={() => seekBy(-10)} className="rounded-lg bg-[#151A36] px-3 py-2"><Text className="text-xs font-bold text-white">↶ 10 сек</Text></Pressable><Pressable accessibilityRole="button" onPress={() => seekBy(10)} className="rounded-lg bg-[#151A36] px-3 py-2"><Text className="text-xs font-bold text-white">10 сек ↷</Text></Pressable>{[0.8, 1, 1.2].map((rate) => <Pressable key={rate} accessibilityRole="button" onPress={() => setPlaybackRate(rate)} className={`rounded-lg px-3 py-2 ${playbackRate === rate ? "bg-[#A993FF]" : "bg-[#151A36]"}`}><Text className="text-xs font-bold text-white">{rate.toFixed(1)}×</Text></Pressable>)}</View></View> : null}
        </View>
