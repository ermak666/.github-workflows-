  ...
  return (
    <ScreenContainer className="px-5">
      <ScrollView contentContainerStyle={{ paddingBottom: 40 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        ...
        {runError ? <View className="mt-3 rounded-xl bg-[#FFF1E8] p-3"><Text className="text-xs font-bold text-warning">КОМАНДА НЕ ВЫПОЛНЕНА</Text><Text className="mt-2 text-sm font-semibold text-warning">{runError}</Text></View> : null}
        ...
        <View className="mt-4 rounded-3xl border border-primary bg-[#E9EAFE] p-4"><Text className="text-base font-bold text-[#171A33]">ИИ-помощник по коду</Text><Text className="mt-1 text-sm leading-5 text-[#42446F]">Он не запускает ваш код. Помощник читает учебный фрагмент, ищет понятные ошибки и подсказывает следующий шаг.</Text><Pressable disabled={coach.isPending} onPress={askCoach} style={({ pressed }) => ({ opacity: pressed || coach.isPending ? 0.7 : 1 })} className="mt-3 items-center rounded-xl bg-primary py-3"><Text className="font-bold text-white">{coach.isPending ? "Разбираю код…" : "Попросить подсказку"}</Text></Pressable>{coachMessage ? <Text style={{ fontSize: 14 * fontScale, lineHeight: 21 * fontScale }} className="mt-3 text-[#171A33]">{coachMessage}</Text> : null}</View>
